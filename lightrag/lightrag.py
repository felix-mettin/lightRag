...
class LightRAG:
    ...
    kg_schema_mode: str = field(
        default=get_env_value("KG_SCHEMA_MODE", "electrical_controlled", str)
    )
    """Knowledge graph extraction mode."""

    schema_extract_max_retries: int = field(
        default=get_env_value("SCHEMA_EXTRACT_MAX_RETRIES", 2, int)
    )
