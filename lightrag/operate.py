...
# Import from rule submodules (rule.py shim is deprecated)
from .rule.workflows import SchemaExtractionWorkflow, QueryAugmentationWorkflow
from .rule.utils import (
    ...
    _normalize_text_key,
    _stable_test_id,
    _classify_query_value_resolution_mode,
)
from .rule.query_processing import _resolve_chunk_id
from .rule.extraction import (
    _upsert_controlled_node,
    _upsert_controlled_edge,
    _validate_controlled_payload,
    _build_controlled_nodes_edges,
)
from .rule.query_processing import (
    _should_bypass_query_cache,
    _extract_doc_type_filters,
    _filter_search_result_by_doc_type,
    _build_test_item_display_map,
    _extract_current_report_scopes,
    _filter_project_context_by_report_scope,
    _filter_context_by_final_test_item_scope,
    _get_display_param_suppressions,
    _build_resolved_rule_overrides,
    _build_final_test_item_scope,
    _build_scope_focused_query,
    _postprocess_electrical_markdown_response,
    _log_electrical_answer_debug,
)
from .rule.domain_rules import (
    _evaluate_domain_rule_decisions,
    _apply_domain_rule_decisions_to_project_context,
)
            ...
            async def _extract_with_retries(content_text: str) -> tuple[list, list, str | None]:
                ...
                for attempt in range(schema_extract_max_retries + 1):
                    ...
                    try:
                        ...
                        # Use SchemaExtractionWorkflow for structured extraction
                        extraction_workflow = SchemaExtractionWorkflow(global_config)
                        # Update chunk_meta with clause info parsed from content
                        if not chunk_meta.get("clause_id"):
                            clause_id, clause_title = extraction_workflow.parse_clause_info(
                                content_text, chunk_meta
                            )
                            chunk_meta["clause_id"] = clause_id
                            chunk_meta["clause_title"] = clause_title
                        nodes, edges = await extraction_workflow.process_extraction(
                            result or "", content_text, chunk_key, file_path
                        )
...
async def _build_context_str(
    ...
    # Initialize QueryAugmentationWorkflow for electrical_controlled mode
    query_workflow = None
    if global_config.get("kg_schema_mode") == "electrical_controlled":
        query_workflow = QueryAugmentationWorkflow(global_config)
    
    # Evaluate domain rules for electrical_controlled mode
    if query_workflow is not None:
        domain_rule_decisions = query_workflow.evaluate_domain_rules(rule_query_text)
    else:
        domain_rule_decisions = _evaluate_domain_rule_decisions(rule_query_text, schema_cfg)
    
    # Extract model and rating information from query text
    ...
    if query_workflow is not None:
        project_param_map, project_param_value_map = query_workflow.build_project_context(
            rule_query_text, domain_rule_decisions
        )
    else:
    ...
    # Extract report scopes from query
    if query_workflow is not None:
        current_report_scopes = query_workflow.extract_report_scopes(rule_query_text)
    else:
    ...
    if current_report_scopes:
        ...
        if query_workflow is not None:
            project_param_map, project_param_value_map = query_workflow.filter_by_report_scope(
                project_param_map, project_param_value_map, current_report_scopes
            )
        else:
    ...
    # Build resolved rule overrides and final test item scope
    if query_workflow is not None:
        resolved_rule_overrides = query_workflow.build_resolved_rule_overrides(domain_rule_decisions)
        allowed_final_test_items, removed_test_items = query_workflow.build_final_test_item_scope(
    ...
    if query_workflow is not None:
        test_item_display_map = query_workflow.build_test_item_display_map(display_project_param_map)
        entities_context, relations_context = query_workflow.filter_context_by_scope(
